...
(docker stop stochsscontainer1_7 >> .uninstallLog 2>&1 && docker rm stochsscontainer1_7) || (echo "Could not remove StochSS container version 1.7. Exiting.." && exit -1)
num_containers=`docker ps -aq | wc -l`
if [[ `echo $num_containers` == 0 ]]
then 
	echo "Safe to delete VM."
